...
export const RegisterForm: React.FC<RegisterFormProps> = ({
  ...
  useEffect(() => {
    if (initialData && isEditing) {
      setFormData({
        date: initialData.Fecha.toLocaleDateString("es-ES", {
          day: "2-digit",
          month: "2-digit",
          year: "numeric",
        }),
  ...
  const handleCsvSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (savedCount > 0) {
        toast.success(
          `${savedCount} registro(s) guardado(s) exitosamente. Ve al Historial para verlos.`,
          {
            duration: 5000,
          },
        );
        if (errorCount > 0) {
          toast.warning(
            `Hubo ${errorCount} líneas que no se pudieron procesar.`,
          );
        }
        setCsvContent("");
        ...
        toast.error(
          `Error: No se guardaron registros. ${errorCount} líneas fallaron.`,
        );
      } else {
  ...
  const inputClassName =
    "w-full px-3 py-2 bg-[#0a1310] border border-[#1e3327] rounded-lg text-white placeholder-gray-600 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30 transition-colors";
  ...
  const SectionHeader = ({
    ...
    <div className="flex items-center gap-2 border-t border-[#1e3327] pt-5 mb-4">
    ...
    <div className="space-y-6">
      ...
      {(activeTab === "manual" || isEditing) && (
        <form onSubmit={handleManualSubmit} className="space-y-2">
          ...
          <SectionHeader icon={Scale} label="Datos Basicos" />
          ...
          {/* Section 2: Composicion Corporal */}
          <SectionHeader icon={Activity} label="Composicion Corporal" />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">
                Grasa Corporal (kg)
          ...
          {/* Mobile toggle for advanced sections */}
          <div className="lg:hidden">
            <button
              type="button"
              onClick={() => setShowAdvanced(!showAdvanced)}
              className="flex items-center gap-2 w-full px-4 py-2.5 mt-4 bg-[#111c16] border border-[#1e3327] rounded-lg text-sm font-medium text-gray-300 hover:text-white hover:bg-[#162119] transition-colors"
            >
              <ChevronDown
          ...
          {/* Section 3: Hidratacion y Huesos (hidden on mobile unless toggled) */}
          <div className={`${showAdvanced ? "block" : "hidden"} lg:block`}>
            <SectionHeader icon={Droplets} label="Hidratacion y Huesos" />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">
                  Agua Corporal (kg)
            ...
            {/* Section 4: Metabolismo y Bioimpedancia */}
            <SectionHeader icon={Zap} label="Metabolismo y Bioimpedancia" />
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">
        ...
        <form onSubmit={handleCsvSubmit} className="space-y-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mb-3">
              Puedes pegar el contenido del CSV o cargar un archivo. Formato
              esperado: Date,Weight (kg),BMI,Fat Mass (kg),...
            </p>
